...
def import_social_network(numberNodes, probConnection):
    ...
    try:
        ...
        file.close()
        print("\nLoading social network file,\n\nNumber of nodes: {}\nProability of having an edge between two nodes: {}".format(numberNodes, probConnection))
        ...
        if type(SN.list_edges[1]) != Edge:
            try:
                SN = import_social_network(numberNodes, probConnection)
            except:
...
def spread_message_SN(SN : Graph, typeMessage, list_seeds_nodes, multipletimeinfluenced = True, multipletimesharing = False):
    queue = Queue()
    activated_nodes = []
    total_messages = 0
    # activating seeds
    ...
    while not queue.empty():
        ...
        for edge_id in SN.adjacent_matrix[node_id]:
            ...
            ##FIRST CASE multi_influenced==True, multi_sharing==False.
            ##OUR BASE CASE, we'll develop this idea
            if multipletimeinfluenced == True and multipletimesharing == False:
            ...
            ##SECOND CASE multi_influenced==False, multi_sharing==False.
            elif multipletimeinfluenced == False and multipletimesharing == False:
                None #Constructing
            ##THIRD CASE multi_influenced==False, multi_sharing==True.
    ...
    print("Total number of nodes in the SN: {}".format(len(SN.list_nodes)))
    print("Total number of edges in the SN: {}".format(len(SN.list_edges)))
...
# erase = '\x1b[1A\x1b[2K'
#SN = import_social_network(100,0.1)
#spread_message_SN(SN, "A", [1])
